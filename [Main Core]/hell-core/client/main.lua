...
Citizen.CreateThread(function()
	while true do
		...
		if paycheckReady == true and stackTwice == false and stackThree == false then
			GRPCore.TriggerServerCallback('paycheck:checkSalary', function(result)
...
AddEventHandler('grp:playerLoaded', function(xPlayer)
	GRPCore.PlayerLoaded = true
	GRPCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		GRPCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			GRPCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			GRPCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = GRPCore.Math.GroupDigits(v.money)
			})
		end
		...
		GRPCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		GRPCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	while not GRPCore.PlayerLoaded do
		Citizen.Wait(1)
	end
	...
	-- Restore position
	if GRPCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, GRPCore.PlayerData.lastPosition.x, GRPCore.PlayerData.lastPosition.y, GRPCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('grp:restoreLoadout') -- restore loadout
...
AddEventHandler('grp:onPlayerDeath', function()
	isDead = true
end)
...
AddEventHandler('skinchanger:modelLoaded', function()
	...
	TriggerEvent('grp:restoreLoadout')
end)

RegisterNetEvent('grp:setAccountMoney')
AddEventHandler('grp:setAccountMoney', function(account)
	for k,v in ipairs(GRPCore.PlayerData.accounts) do
		if v.name == account.name then
			GRPCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		GRPCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = GRPCore.Math.GroupDigits(account.money)
		})
	end
end)
...
AddEventHandler('es:activateMoney', function(money)
	GRPCore.PlayerData.money = money
end)
...
-- Commands
RegisterNetEvent('grp:teleport')
AddEventHandler('grp:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('grp:loadIPL')
AddEventHandler('grp:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('grp:unloadIPL')
AddEventHandler('grp:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('grp:playAnim')
AddEventHandler('grp:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('grp:playEmote')
AddEventHandler('grp:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('grp:spawnVehicle')
AddEventHandler('grp:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	GRPCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('grp:spawnObject')
AddEventHandler('grp:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	GRPCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('grp:pickup')
AddEventHandler('grp:pickup', function(id, label, player)
	...
	--[[GRPCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('grp:removePickup')
AddEventHandler('grp:removePickup', function(id)
	GRPCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('grp:spawnPed')
AddEventHandler('grp:spawnPed', function(model)
...
RegisterNetEvent('grp:deleteVehicle')
AddEventHandler('grp:deleteVehicle', function()
	local playerPed = PlayerPedId()
	local vehicle   = GRPCore.Game.GetVehicleInDirection()
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance <= 1.0 and not v.inRange and IsPedOnFoot(playerPed) and IsControlJustReleased(0, Keys["E"]) then
				...
				TriggerServerEvent('grp:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if GRPCore.PlayerLoaded and isPlayerSpawned then
			local coords = GetEntityCoords(playerPed)

			if not IsEntityDead(playerPed) then
				GRPCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end

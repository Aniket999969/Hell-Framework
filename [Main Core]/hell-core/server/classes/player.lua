function CreateExtendedPlayer(player, accounts, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		money = GRPCore.Math.Round(money)

		if money >= 0 then
			self.player.setMoney(money)
		else
			print(('grp-core: %s attempted exploiting! (reason: player tried setting -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.setBankBalance = function(money)
		...
		if money >= 0 then
			...
			print(('grp-core: %s attempted exploiting! (reason: player tried setting -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			...
			print(('grp-core: %s attempted exploiting! (reason: player tried adding -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.removeMoney = function(money)
		...
		if money >= 0 then
			...
			print(('grp-core: %s attempted exploiting! (reason: player tried removing -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.addBank = function(money)
		...
		if money >= 0 then
			...
			print(('grp-core: %s attempted exploiting! (reason: player tried adding -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.removeBank = function(money)
		...
		if money >= 0 then
			...
			print(('grp-core: %s attempted exploiting! (reason: player tried removing -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.getLastPosition = function()
		if self.lastPosition and self.lastPosition.x and self.lastPosition.y and self.lastPosition.z then
			self.lastPosition.x = GRPCore.Math.Round(self.lastPosition.x, 1)
			self.lastPosition.y = GRPCore.Math.Round(self.lastPosition.y, 1)
			self.lastPosition.z = GRPCore.Math.Round(self.lastPosition.z, 1)
		end
	...
	self.setAccountMoney = function(acc, money)
		if money < 0 then
			print(('grp-core: %s attempted exploiting! (reason: player tried setting -1 account balance)'):format(self.identifier))
			return
		end

		local account   = self.getAccount(acc)
		local prevMoney = account.money
		local newMoney  = GRPCore.Math.Round(money)
		...
		TriggerClientEvent('grp:setAccountMoney', self.source, account)
	end

	self.addAccountMoney = function(acc, money)
		if money < 0 then
			print(('grp-core: %s attempted exploiting! (reason: player tried adding -1 account balance)'):format(self.identifier))
			return
		end

		local account  = self.getAccount(acc)
		local newMoney = account.money + GRPCore.Math.Round(money)
	...
	self.removeAccountMoney = function(acc, money)
		if money < 0 then
			print(('grp-core: %s attempted exploiting! (reason: player tried removing -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.setJob = function(job, grade)
		...
		if GRPCore.DoesJobExist(job, grade) then
			...
			TriggerEvent('grp:setJob', self.source, self.job, lastJob)
			TriggerClientEvent('grp:setJob', self.source, self.job)
		else
			print(('grp-core: ignoring setJob for %s due to job not found!'):format(self.source))
		end
	end
	return self
end

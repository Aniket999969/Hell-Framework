GRPCore.StartPayCheck = function()
		...
		for i=1, #xPlayers, 1 do
			...
			if salary > 0 then
				...
				elseif Config.EnableSocietyPayouts then -- possibly a society
					TriggerEvent('grp-society:getSociety', xPlayer.job.name, function (society)
						if society ~= nil then -- verified society
							TriggerEvent('grp-addonaccount:getSharedAccount', society.account, function (account)
								if account.money >= salary then -- does the society money to pay its employees?
									...
									TriggerClientEvent('grp:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
								end
							end)
						else -- not a society
							xPlayer.addAccountMoney('bank', salary)
...
GRPCore.RegisterServerCallback('paycheck:checkSalary', function(source, cb)
	if source ~= nil then
				...
				if society ~= nil then -- verified society
					TriggerEvent('grp-addonaccount:getSharedAccount', society.account, function (account)
						if account.money >= salary then -- does the society money to pay its employees?
							...
							TriggerClientEvent('grp:showAdvancedNotification', source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
						end
					end)
				else -- not a society
					xPlayer.addMoney( salary)

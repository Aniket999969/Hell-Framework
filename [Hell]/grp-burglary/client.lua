...
Citizen.CreateThread(function ()
  ...
  while GRPCore.GetPlayerData() == nil do
    Citizen.Wait(10)
  end
...
AddEventHandler('houseRobberies:attempt', function()
 ...
 if exports['grp-inventory']:hasEnoughOfItem('advlockpick', 1, false) and isNight() and not isRobbing then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    exports["grp-taskbar"]:taskBar(12000, 'Lockpicking Property')
     isLockpicking = false
     pedSpawned = false

     --if math.random(1, 10) >= 1 then
     if math.random(5, 10) == 1 then
      TriggerEvent('grp-dispatch:houserobbery')
     end
 ...
 elseif exports['grp-inventory']:hasEnoughOfItem('advlockpick', 0, false) and not isRobbing then
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     ...
     if IsControlJustReleased(1, 47) then
      ...
      exports["grp-taskbar"]:taskBar(20000, "Searching "..myRobbableItems[i]['name'])

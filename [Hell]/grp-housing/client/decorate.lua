...
RegisterNetEvent("grp-housing:client:decorate")
AddEventHandler("grp-housing:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("grp-housing:server:sethousedecorations")
AddEventHandler("grp-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		GRPCore.TriggerServerCallback('grp-housing:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("grp-housing:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		GRPCore.ShowNotification('Speed is '.. tostring(speeds[curSpeed]))
	end

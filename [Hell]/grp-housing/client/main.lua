...
local servername = 'GRP Development'
local cam = nil
local viewCam = false
...
RegisterNetEvent('grp-housing:client:sellHouse')
AddEventHandler('grp-housing:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('grp-housing:server:viewHouse', closesthouse)
    end
end)
...
AddEventHandler('grp:playerLoaded', function()
    TriggerServerEvent('grp-housing:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('grp-housing:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerServerEvent("grp-housing:server:setHouses")
end)

RegisterNetEvent('grp-housing:client:setHouseConfig')
AddEventHandler('grp-housing:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
    --TriggerEvent("grp-housing:client:refreshHouse")
	
end)

RegisterNetEvent('grp-housing:client:lockHouse')
AddEventHandler('grp-housing:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('grp-housing:client:createHouses')
AddEventHandler('grp-housing:client:createHouses', function(price, tier)
    ...
    TriggerServerEvent('grp-housing:server:addNewHouse', area, coords, price, tier)
end)

RegisterNetEvent('grp-housing:client:toggleDoorlock')
AddEventHandler('grp-housing:client:toggleDoorlock', function()
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('grp-housing:server:lockHouse', false, closesthouse)
				TriggerEvent('DoLongHudText', 'The Door is unlocked.', 1)
            else
                TriggerServerEvent('grp-housing:server:lockHouse', true, closesthouse)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("grp-housing:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                            ...
                            if not inOwned then
                                if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    if not Config.Houses[closesthouse].locked then
                                        ...
                                        if IsControlJustPressed(0, Keys["G"]) then
                                            TriggerServerEvent('grp-housing:server:RingDoor', closesthouse)
                                        end
                                    end
                                end
                            elseif inOwned then
                    ...
                    if closesthouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('grp-login:swapcharacter')
                                    leaveOwnedHouse(closesthouse)
                                    showClothing = false
                                end
...
RegisterNetEvent('grp-housing:client:RingDoor')
AddEventHandler('grp-housing:client:RingDoor', function(player, house)
...
RegisterNetEvent('grp-housing:client:giveHouseKey')
AddEventHandler('grp-housing:client:giveHouseKey', function(data)
    local player, distance = GRPCore.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('grp-housing:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('grp-housing:client:removeHouseKey')
AddEventHandler('grp-housing:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            GRPCore.TriggerServerCallback('grp-housing:server:getHouseOwner', function(result)
                if GRPCore.GetPlayerData().identifier == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
...
RegisterNetEvent('grp-housing:client:refreshHouse')
AddEventHandler('grp-housing:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('grp-housing:client:setstash')
AddEventHandler('grp-housing:client:setstash', function(data)
ExecuteCommand("setlocation stash")
end)

RegisterNetEvent('grp-housing:client:setstash2')
AddEventHandler('grp-housing:client:setstash2', function(data)
ExecuteCommand("setlocation stash2")
end)

RegisterNetEvent('grp-housing:client:setoutfit')
AddEventHandler('grp-housing:client:setoutfit', function(data)
ExecuteCommand("setlocation outfit")
end)

RegisterNetEvent('grp-housing:client:setlogout')
AddEventHandler('grp-housing:client:setlogout', function(data)
ExecuteCommand("setlocation logout")
end)

RegisterNetEvent('grp-housing:client:SpawnInApartment')
AddEventHandler('grp-housing:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function removeHouseKey(citizenData)
    TriggerServerEvent('grp-housing:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['grp-interior']:CreateTier1House(coords)
	elseif Config.Houses[house].tier == 2 then
        data = exports['grp-interior']:CreateMichaelShell(coords)
	elseif Config.Houses[house].tier == 3 then
        data = exports['grp-interior']:CreateTrevorsShell(coords)
	elseif Config.Houses[house].tier == 4 then
        data = exports['grp-interior']:CreateFranklinShell(coords)
	elseif Config.Houses[house].tier == 5 then
        data = exports['grp-interior']:CreateFranklinAuntShell(coords)
	elseif Config.Houses[house].tier == 6 then
        data = exports['grp-interior']:CreateApartmentShell(coords)
	elseif Config.Houses[house].tier == 7 then
        data = exports['grp-interior']:CreateCaravanShell(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    TriggerEvent('grp-weathersync:client:DisableSync')
    TriggerEvent('grp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('grp-housing:client:enterOwnedHouse')
AddEventHandler('grp-housing:client:enterOwnedHouse', function(house)
	enterOwnedHouse(house)
end)

RegisterNUICallback('HasEnoughMoney', function(data, cb)
    GRPCore.TriggerServerCallback('grp-housing:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('grp-housing:client:LastLocationHouse')
AddEventHandler('grp-housing:client:LastLocationHouse', function(houseId)
...
function leaveOwnedHouse(house)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    exports['grp-interior']:DespawnInterior(houseObj, function()
        UnloadDecorations()
        TriggerEvent('grp-weathersync:client:EnableSync')
        Citizen.Wait(250)
        DoScreenFadeIn(250)
        ...
        inside = false
        TriggerEvent('grp-weed:client:leaveHouse')
        showClothing = false
    end)
end
...
function enterNonOwnedHouse(house)
    ...
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerServerEvent('grp-housing:server:SetInsideMeta', house, true)
    TriggerEvent('grp-weathersync:client:DisableSync')
    TriggerEvent('grp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
    SetWeatherTypePersist('EXTRASUNNY')
...
function leaveNonOwnedHouse(house)
    ...
    exports['grp-interior']:DespawnInterior(houseObj, function()
        ...
        showClothing = false
        TriggerEvent('grp-weed:client:leaveHouse')
        TriggerServerEvent('grp-housing:server:SetInsideMeta', house, false)
    end)
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    GRPCore.TriggerServerCallback('grp-housing:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
RegisterNetEvent('grp-housing:client:setupHouseBlips')
AddEventHandler('grp-housing:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            GRPCore.TriggerServerCallback('grp-housing:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('grp-housing:client:SetClosestHouse')
AddEventHandler('grp-housing:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('grp-housing:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('grp-housing:client:viewHouse')
AddEventHandler('grp-housing:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            GRPCore.TriggerServerCallback('grp-housing:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            GRPCore.TriggerServerCallback('grp-housing:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        GRPCore.TriggerServerCallback('grp-housing:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('grp-housing:client:setLocation')
AddEventHandler('grp-housing:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'stash' then
                TriggerServerEvent('grp-housing:server:setLocation', coords, closesthouse, 1)
            elseif data == 'outift' then
                TriggerServerEvent('grp-housing:server:setLocation', coords, closesthouse, 2)
            elseif data == 'logout' then
                TriggerServerEvent('grp-housing:server:setLocation', coords, closesthouse, 3)
			elseif data == 'stash2' then
                TriggerServerEvent('grp-housing:server:setLocation', coords, closesthouse, 4)
            end
        else
...
RegisterNetEvent('grp-housing:client:refreshLocations')
AddEventHandler('grp-housing:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse == house then
        if inside then
            TriggerEvent('grp-housing:getid', closesthouse, house)
            if type == 1 then

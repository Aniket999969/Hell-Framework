GRP = GRP or {}
GRP.Doors = GRP.Doors or {}
GRP.DoorCoords = GRP.DoorCoords or {}
GRP.offSet = GRP.offSet or {}
...
function GRP.alterState(alterNum)

    if GRP.DoorCoords[alterNum]["lock"] == 0 then 
        GRP.DoorCoords[alterNum]["lock"] = 1 
    else 
        GRP.DoorCoords[alterNum]["lock"] = 0 
    end
    TriggerClientEvent("GRP:Door:alterState",-1,alterNum,GRP.DoorCoords[alterNum]["lock"])

end

RegisterNetEvent( 'GRP:Door:alterState' )
AddEventHandler( 'GRP:Door:alterState', function(alterNum,num)
	GRP.DoorCoords[alterNum]["lock"] = num
end)

RegisterNetEvent("grp-doors:alterlockstateclient")
AddEventHandler("grp-doors:alterlockstateclient", function(doorCoords)
    GRP.DoorCoords = doorCoords
end)

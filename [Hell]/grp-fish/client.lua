    ...
    while true do
        ...
        if dis <= 5 then
            ...
            if IsControlJustReleased(0, 38) then
                local finished = exports["grp-taskbar"]:taskBar(2000,"Selling Fish",true,false,playerVeh)
        ...
        if dis3 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    if GRPCore.GetPlayerData().money >= 500 then
                    TriggerServerEvent('fish:checkAndTakeDepo')
                    Citizen.Wait(500)
                        canSpawn = false
                        GRPCore.Game.SpawnVehicle('marquis', vector3(-3448.48, 971.98, 1.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
                        end)
                    elseif GRPCore.GetPlayerData().money < 500 then
        ...
        if dis4 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        GRPCore.Game.SpawnVehicle('suntrap', vector3(1299.69, 4194.82, 30.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
        ...
        if dis5 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        GRPCore.Game.SpawnVehicle('tropic', vector3(3865.89, 4476.66, 1.53), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
...
RegisterNetEvent('grp-fish:lego')
AddEventHandler('grp-fish:lego', function()
...
function StartFish()
    ...
    if IsEntityInWater(boat) and IsPedSwimming(ply) == false and inZone == true then
        if exports["grp-inventory"]:hasEnoughOfItem('fishingrod',1,false) then
            isFishing = true
            cancel = false
            Fish()
        end
...
function Catch()
    if cancel == false then
        ...
        local finished = exports["grp-taskbarskill"]:taskBar(2000,math.random(7,14))
        if finished == 100 then
            isFishing = false
            ...
            elseif rdn > 11 then
                ...
                TriggerServerEvent('grp-fish:getFish')
                SetCurrentPedWeapon(ply, `WEAPON_UNARMED`, true)
                ClearPedTasksImmediately(ply)
            end
...
function SellItems()
    local sellfish = math.random(40, 80)
        if exports["grp-inventory"]:hasEnoughOfItem("fish",2,false) then 
        TriggerEvent("inventory:removeItem", "fish", 2)
        TriggerServerEvent('grp-fish:payShit', sellfish)
    else

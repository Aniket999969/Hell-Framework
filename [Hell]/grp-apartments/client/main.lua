            ...
            if InApartment then
                ...
                if IsOwned then
                    ...
                    if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y,Apartments.Locations[ClosestHouse].coords.enter.z, true) < 1.2)then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            GRPCore.TriggerServerCallback('apartments:GetOwnedApartment', function(result)
                                if result ~= nil then
...
function apartal()
	...
	TriggerEvent("grp-uimenu:createMenu", myListMenu, function(data)
...
RegisterNetEvent('grp-apartments:client:LastLocationHouse')
AddEventHandler('grp-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    GRPCore.TriggerServerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            GRPCore.TriggerServerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                data = exports['grp-interior']:CreateApartmentFurnished(coords)
                Citizen.Wait(100)
                houseObj = data[1]
                POIOffsets = data[2]
        ...
        if new ~= nil then
            if new then
                TriggerEvent('grp-interior:client:SetNewState', true)
            else
                TriggerEvent('grp-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('grp-interior:client:SetNewState', false)
        end
...
function SetClosestApartment()
    ...
    if current ~= ClosestHouse  and not InApartment then
        ...
        GRPCore.TriggerServerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    GRPCore.TriggerServerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = GetPlayerPed(-1);
...
function OutfitsLijst()
    GRPCore.TriggerServerCallback('apartments:GetOutfits', function(outfits)
        ped = GetPlayerPed(-1);
        MenuTitle = "Clothing :"
        ClearMenu()

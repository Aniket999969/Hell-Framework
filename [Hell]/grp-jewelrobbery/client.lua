...
RegisterNetEvent('grp-jewelrobbery:policeclosure')
AddEventHandler('grp-jewelrobbery:policeclosure', function()
	policeclosed = true
	storeclosed = false
	IsAbleToRob = false
end)
...
AddEventHandler('grp:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('grp-jewelrobbery:loadconfig')
end)

RegisterNetEvent('grp-jewelrobbery:resetcases')
AddEventHandler('grp-jewelrobbery:resetcases', function(list)
...
RegisterNetEvent('grp-jewelrobbery:setcase')
...
RegisterNetEvent('grp-jewelrobbery:loadconfig')
AddEventHandler('grp-jewelrobbery:loadconfig', function(casestatus)
...
RegisterNetEvent('grp-jewelrobbery:playsound')
...
AddEventHandler('grp-jewelrobbery:EnteredArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
AddEventHandler('grp-jewelrobbery:LeftArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			RemoveModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			TriggerEvent('grp-jewelrobbery:EnteredArea')
			shockingevent = false
		...
		if not IsInArea and HasAlreadyEnteredArea then
			TriggerEvent('grp-jewelrobbery:LeftArea')
			HasAlreadyEnteredArea = false
			shockingevent = false
			IsAbleToRob = false
			storeclosed = false
...
function CheckPolice()
	if CopsOnline ~= exports["grp-jobnumbers"]:jobonline('cops') then
		HasAlreadyEnteredArea = false
	end
	CopsOnline = exports["grp-jobnumbers"]:jobonline('cops')
	return exports["grp-jobnumbers"]:jobonline('cops')
end
						...
						if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
							...
							if breakchance <= robalbe.chance then
								if policenotify <= Config.PoliceNotifyBroken and not HasNotified then
									local playerCoords = GetEntityCoords(PlayerPedId())
									TriggerServerEvent('grp-jewelrobbery:policenotify')
									HasNotified = true
								end
								Citizen.Wait(2100)
								TriggerServerEvent('grp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'break')
								CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z,  0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
								ClearPedTasksImmediately(ply)
								TriggerServerEvent("grp-jewelrobbery:setcase", i, true)	
							else
								Citizen.Wait(2100)
								TriggerServerEvent('grp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'nonbreak')
								ClearPedTasksImmediately(ply)
        ...
        if distance < 3.0 then
        	Wait(1)
             ...
             if IsControlJustReleased(0,38) and distance < 1.0 then
             	if exports["grp-inventory"]:hasEnoughOfItem("Gruppe6Card3",1,false) then
             		TriggerEvent("inventory:removeItem", "Gruppe6Card3", 1)
             		TriggerServerEvent("grp-doors:alterlockstate",61)
             		TriggerServerEvent("grp-doors:alterlockstate",62)
             	end
             end
		else
            Wait(3000)

...
$('.create-button').click(function () {
    ...
    $.post("http://grp-characters/CharacterChosen", JSON.stringify({
...
$("#deletechar").click(function () {
	...
	if (chosenCharacterId > 0) {
		$.post("http://grp-characters/DeleteCharacter", JSON.stringify({
			charid: Number(chosenCharacterId),
		}));
    }
    ...
    window.onload = function(e) {
        ...
        document.onkeydown = function(data) {
            if (data.which == 13 && IsInMainMenu) {
                ...
                $.post("http://grp-characters/ShowSelection", JSON.stringify({}));
                ShowLoading(true)
            }
        }
    }
...
function disconnect(){
    $.post("http://grp-characters/DisconnectGame", JSON.stringify({
    }));

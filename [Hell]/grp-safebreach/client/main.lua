...
AddEventHandler("grp:playerLoaded", function()
	...
	GRPCore.TriggerServerCallback("safebreach:checkSafeBreaches", function(successfull)
		if successfull then
				...
				for objectIndex = 1, #eventData["objects"] do
					if NetworkDoesEntityExistWithNetworkId(eventData["objects"][objectIndex]) then
						...
						if dstCheck <= 2.0 then
							...
							GRPCore.Game.Utils.DrawText3D(GetEntityCoords(breachObject), "[~g~G~s~]", 0.4)
						end
					end
				end
			...
			if dstCheck <= 2.5 then 
				...
				if display then
					...
					if dstCheck <= 1.0 then
						...
						GRPCore.ShowHelpNotification(displayText)

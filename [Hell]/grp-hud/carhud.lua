        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["drp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
AddEventHandler("drp-jobmanager:playerBecameJob", function(job, name)
...
RegisterNetEvent('drp-voice:focus:set')
AddEventHandler('drp-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent("drp-admin:currentDebug")
AddEventHandler("drp-admin:currentDebug", function(debugToggle)

...
$(document).ready(function() {
    ...
    $.post('http://grp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://grp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://grp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://grp-phone/newPostSubmit', JSON.stringify({
                ...
                case "newCPostSubmit":
                    e.preventDefault();
                    $.post('http://grp-phone/newCPostSubmit', JSON.stringify({
                ...
                case "group-manage":
                    ...
                    $.post('http://grp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://grp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://grp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://grp-phone/race:completed', JSON.stringify({
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://grp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://grp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://grp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://grp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://grp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://grp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://grp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://grp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://grp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://grp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/btnAttemptHouseSale', JSON.stringify());

});

$("#real-estate-create-form").submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/btnAttempCreateHouse', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/btnTransferHouse', JSON.stringify({
...
$('#real-estate-transfers-form').submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/btnSellingHousePlayer', JSON.stringify({
...
$('#home-owner-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/btnTransferHouse2', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://grp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://grp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://grp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://grp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://grp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://grp-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://grp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://grp-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://grp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://grp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://grp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://grp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://grp-phone/btnEvictHouse', JSON.stringify({}));
    $('#real-estate-evict-modal-').modal('close');
});

$('#real-estate-delete-modal-accept').click(function () {
    $.post('http://grp-phone/btnDeleteHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://grp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://grp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://grp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://grp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://grp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://grp-phone/removeHouseKey', JSON.stringify({
...
$('.criminal-pages-entries').on('click', '.pidtrash', function () {
    $.post('http://grp-phone/deleteCPpost', JSON.stringify({
        adv: $(this).data('adv'),
        name: '',
        number: $(this).data('number')
    }));
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://grp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://grp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://grp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://grp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://grp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://grp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://grp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://grp-phone/loadUserGPS', 
    JSON.stringify({ 
    house_id: $(this).data('house-id'), 
...
$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://grp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://grp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));

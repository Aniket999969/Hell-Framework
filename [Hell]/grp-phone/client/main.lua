...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = GRPCore.PlayerData.job.grade
    local rankConverted = GRPCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = GRPCore.PlayerData.job.label,
...
function findVehFromPlateAndLocate(plate)
  ...
  local gameVehicles = GRPCore.Game.GetVehicles()
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("grp-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["grp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["grp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["grp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["grp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["grp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["grp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("grp:heHasPhone")
    end
  end
end)

RegisterNetEvent('grp:heHasPhone')
AddEventHandler('grp:heHasPhone', function()
    GotPhone()
end)
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if GRPCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if GRPCore.PlayerData.job.name ~= "taxi" and GRPCore.PlayerData.job.name ~= "towtruck" and GRPCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', GRPCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = GRPCore.PlayerData.job.name .. ', ' .. GRPCore.PlayerData.job.grade_label,
...
RegisterNetEvent("grp-phone:userTwitter")

AddEventHandler("grp-phone:userTwitter", function(user)
  userTwitter = user
 end)

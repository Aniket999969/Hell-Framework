...
function cmd.RunCommand(caller, args)
    ...
    --GRP.Admin:SetRank(caller.target, caller.rank)
end
...
function cmd.DrawCommand()
        ...
        if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then GRP.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end) end           

        if WarMenu.Button("Select a rank", "Selected: " .. (cmd.vars.rank ~= nil and tostring(cmd.vars.rank) or "None")) then GRP.Admin.Menu:DrawRanks(cmd.command, function(_rank)
            cmd.vars.rank = _rank
        end) end
        ...
        if args.target and args.rank then if WarMenu.Button("Set " .. args.target.name .. "'s rank to " .. args.rank) then GRP.Admin:GetCommandData(cmd.command).runcommand(args) cmd.vars.target = nil cmd.vars.rank = nil end end
end

GRP.Admin:AddCommand(cmd)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a ban length", "Length: " .. (cmd.vars.time and cmd.vars.time or "No Time Given")) then
        GRP.Admin.Menu:DrawTextInput(cmd.vars.time and cmd.vars.time or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end

                local timeTable, timeSum, addedTime = GRP.Admin:GetBanTimeFromString(result)
    ...
    if WarMenu.Button("Enter a reason", "Reason: " .. (cmd.vars.reason and cmd.vars.reason or "No Reason Given")) then
        GRP.Admin.Menu:DrawTextInput(cmd.vars.reason and cmd.vars.reason or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.target and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.target.name) then GRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.target = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a Steam ID", "Entered: " .. (cmd.vars.steamid and cmd.vars.steamid or "None")) then 
        GRP.Admin.Menu:ShowTextEntry("Enter a Steam ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.steamid then if WarMenu.Button("Unban " .. cmd.vars.steamid) then GRP.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid}) cmd.vars.steamid = nil end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Kick " .. cmd.vars.target.name) then GRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("God Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Noclip: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()      
    if WarMenu.Button("Teleport to marker") then GRP.Admin:GetCommandData(cmd.command).runcommand({}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerServerEvent('grp-admin:Cloak', src, caller.toggle)
end
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("playerDropped", function()
            ...
            if cmd.vars.cloaked[src] then
                TriggerServerEvent('grp-admin:Cloak', src, false)
    ...
    RegisterNetEvent("grp-admin:CloakRemote")
    AddEventHandler("grp-admin:CloakRemote", function()
        if GRP.Admin:GetPlayerRank() == "dev" then
            cmd.vars.toggle = not cmd.vars.toggle
            GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle})
        end
    end)

    RegisterNetEvent("grp-admin:Cloak")
    AddEventHandler("grp-admin:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Cloak: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunCommand(caller, args)
	...
	TriggerEvent('grp-hospital:client:RemoveBleed', caller.target.src) 
	TriggerEvent('grp-hospital:client:ResetLimbs', caller.target.src)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("grp-admin:ReviveInDistance")
    end


    if cmd.vars.target then if WarMenu.Button("Revive and Heal " .. cmd.vars.target.name) then GRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
...
function cmd.DrawCommand()      
    ...
    if WarMenu.Button("Enter x", "x: " .. (cmd.vars.x and cmd.vars.x or "0.0")) then
        GRP.Admin.Menu:ShowTextEntry("Enter Item", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            if string.find(result, ",") then
                local resultSplit = GRP.Admin.split(result, ',')
    ...
    if WarMenu.Button("Enter y", "y: " .. (cmd.vars.y and cmd.vars.y or "0.0")) then
        GRP.Admin.Menu:ShowTextEntry("Enter y", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter z", "z: " .. (cmd.vars.z and cmd.vars.z or "0.0")) then
        GRP.Admin.Menu:ShowTextEntry("Enter z", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.x and cmd.vars.y and cmd.vars.z then if WarMenu.Button("Teleport to: " .. "X:["..cmd.vars.x.."] Y:["..cmd.vars.y.."] Z:["..cmd.vars.z.."]") then GRP.Admin:GetCommandData(cmd.command).runcommand({x = cmd.vars.x,y = cmd.vars.y,z = cmd.vars.z}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a message", "Message: " .. (cmd.vars.message and cmd.vars.message or "No Message")) then
        GRP.Admin.Menu:ShowTextEntry("Enter a Message", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Send Message: " .. cmd.vars.message) then GRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    if not caller.message then return end
    local src = source
    TriggerEvent("grp-admin:runSpawnCommand", caller.message)
    local playerName = GetPlayerName(PlayerId())
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a model", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        GRP.Admin.Menu:ShowTextEntry("Enter a model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then 
        if WarMenu.Button("Spawn model: " .. cmd.vars.message) then 
            GRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) 
        end 
    end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter Amount", "Amount: " .. (cmd.vars.amount and cmd.vars.amount or "1")) then
        GRP.Admin.Menu:ShowTextEntry("Enter Amount", "", function(result)
            print(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message.." | "..cmd.vars.amount) then GRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,amount = cmd.vars.amount}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Bring " .. cmd.vars.target.name .. " to you.") then GRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Teleport to " .. cmd.vars.target.name .. "'s position") then GRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
    if cmd.vars.lastPos then if WarMenu.Button("Return to your last position") then GRP.Admin:GetCommandData(cmd.command).runcommand({target = false, retn = true}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    GRP.Admin:Log(log, caller)
    GRP.Admin:Bring(caller,args.target.source)
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Attach: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
RegisterNetEvent('grp-admin:toggleRave')
AddEventHandler('grp-admin:toggleRave', function(toggle)
    toggles = toggle
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Player Blips: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle GRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Fix Current Vehicle") then 
        ...
        GRP.Admin:GetCommandData(cmd.command).runcommand()
    end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter an entity ID") then 
        GRP.Admin.Menu:ShowTextEntry("Enter an entity ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
    ...
    if cmd.vars.ent then if WarMenu.Button("Delete Entity", "Entity: " .. (cmd.vars.ent or "none")) then
        GRP.Admin:GetCommandData(cmd.command).runcommand({ent = cmd.vars.ent})
    end end
end
...
--[[
    function cmd.ChatCommand(args)
        ...
        if not GRP.Admin:RankExists(rank) then return end
        if not GRP.Admin:IsValidUser(target) then return end

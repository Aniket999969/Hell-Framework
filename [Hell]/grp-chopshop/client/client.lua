	...
	while true do
		...
		if (isInMarker) then
            ...
            if beingchopped == false then
                TriggerEvent('grp-chopshop:entered')
            end
		...
		if not isInMarker and HasAlreadyEnteredMarker then
			...
			TriggerEvent('grp-chopshop:exited')
        end
        ...
        if CurrentActionMsg == nil then
            ...
            if not beingchopped then
                if IsControlJustReleased(0, Config.ActionButton) then
                    ...
                    TriggerEvent('grp-chopshop:startchop')
                end
            end
        end
    end
...
function OpenParts()
    ...
    exports["grp-taskbar"]:taskBar(3000, "Opening Driver Door")
    Citizen.Wait(3000)
    SetVehicleDoorOpen(vehicle, 0, false, false)
    exports["grp-taskbar"]:taskBar(3000, "Opening Passenger Door")
    Citizen.Wait(3000)
    SetVehicleDoorOpen(vehicle, 1, false, false)
    if GetEntityBoneIndexByName(vehicle, 'door_dside_r') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Opening Back Left Door")
        Citizen.Wait(3000)
        SetVehicleDoorOpen(vehicle, 2, false, false)
        exports["grp-taskbar"]:taskBar(3000, "Opening Back Right Door")
        Citizen.Wait(3000)
        SetVehicleDoorOpen(vehicle, 3, false, false)
    end
    if GetEntityBoneIndexByName(vehicle, 'bonnet') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Opening Hood")
        Citizen.Wait(3000)
        SetVehicleDoorOpen(vehicle, 4, false, false)
    end
    if GetEntityBoneIndexByName(vehicle, 'boot') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Opening Trunk")
        Citizen.Wait(3000)
...
function RemoveParts()
    local ped = GetPlayerPed(-1)
    local vehicle = GetVehiclePedIsUsing(ped)
    exports["grp-taskbar"]:taskBar(3000, "Removing Driver Door")
    Citizen.Wait(3000)
    SetVehicleDoorBroken(vehicle,0, true)
    exports["grp-taskbar"]:taskBar(3000, "Removing Passenger Door")
    Citizen.Wait(3000)
    SetVehicleDoorBroken(vehicle,1, true)
    if GetEntityBoneIndexByName(vehicle, 'door_dside_r') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Removing Back Left Door")
        Citizen.Wait(3000)
        SetVehicleDoorBroken(vehicle,2, true)
        exports["grp-taskbar"]:taskBar(3000, "Removing Back Right Door")
        Citizen.Wait(3000)
        SetVehicleDoorBroken(vehicle,3, true)
    end
    if GetEntityBoneIndexByName(vehicle, 'bonnet') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Removing Hood")
        Citizen.Wait(3000)
        SetVehicleDoorBroken(vehicle,4, true)
    end
    if GetEntityBoneIndexByName(vehicle, 'boot') ~= -1 then
        exports["grp-taskbar"]:taskBar(3000, "Removing Trunk")
        Citizen.Wait(3000)
        SetVehicleDoorBroken(vehicle,5, true)
    end
end

RegisterNetEvent('grp-chopshop:startchop')
AddEventHandler('grp-chopshop:startchop', function ()
    beingchopped = true
    ...
    for i = 1, #Config.Progress, 1 do
        ...
        if IsPedInAnyVehicle(ped, true) then
            ...
            for key, value in pairs(table2) do
                if IsPedInAnyVehicle(ped, true) then
                    exports["grp-taskbar"]:taskBar(key*250, value)
    ...
    if IsPedInAnyVehicle(ped, true) then
        ...
        if ( GetPedInVehicleSeat( vehicle, -1 ) == ped ) then 
            ...
            local vehicleData = GRPCore.Game.GetVehicleProperties(vehicle)
            canchop = false
            ...
            if canchop == true then
                ...
                TriggerServerEvent('grp-chopshop:addCash')
                beingchopped = false
                return
            else
...
AddEventHandler('grp-chopshop:entered', function ()
...
AddEventHandler('grp-chopshop:exited', function ()
	CurrentActionMsg = nil
end)

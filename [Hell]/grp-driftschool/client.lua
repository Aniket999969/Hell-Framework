        ...
        if insideDriftSchool then
            ...
            if repairDist[1] < 5 and health < 1000.0 and veh ~= 0 and rank == 'drift_school' then
                ...
                if IsControlJustPressed(1, 38) then
                    TriggerServerEvent("grp-driftschool:takemoney", repairCost(veh, health))
                    ...
                    if not isPurchaseSuccessful then
                        ...
                        local finished = exports["grp-taskbarskill"]:taskBar(4000,math.random(5,9))
                        if finished ~= 100 then
                            running = false
                        end
                        if finished == 100 then
                            local finished2 = exports["grp-taskbarskill"]:taskBar(2000,math.random(4,8))
...
RegisterNetEvent('grp-driftschool:tookmoney')
AddEventHandler('grp-driftschool:tookmoney', function(taken)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(500)
        if insideDriftSchool then
            rank = GRPCore.GetPlayerData().job.name
            Citizen.Wait(10000)
        end
    end
end)
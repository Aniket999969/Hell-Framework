...
RegisterCommand('ping', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
        if args[1]:lower() == 'accept' then
            TriggerClientEvent('grp-ping:client:AcceptPing', source)
        elseif args[1]:lower() == 'reject' then
            TriggerClientEvent('grp-ping:client:RejectPing', source)
        else
            local tSrc = tonumber(args[1])
            if source ~= tSrc then
                TriggerClientEvent('grp-ping:client:SendPing', tSrc, name.firstname .. ' ' .. name.lastname, source)
            else
...
RegisterServerEvent('grp-ping:server:SendPingResult')
AddEventHandler('grp-ping:server:SendPingResult', function(id, result)
    local name = getIdentity(source)

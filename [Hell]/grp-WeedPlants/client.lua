...
function MFD:Start()
  ...
  GRPCore.TriggerServerCallback('MF_DopePlant:GetLoginData', function(plants)  
  ...
  if self.CanHarvest or self.PolText then 
    if Utils:GetKeyPressed("E") and (GetGameTimer() - self.timer) > 200 and not self.CurInteracting then
      Citizen.CreateThread(function()
        ...
        exports["grp-taskbar"]:taskBar(20000, "Harvesting")
...
function MFD:TextHandler()
  ...
  local plyData = GRPCore.GetPlayerData()
...
function MFD:SyncCheck()
  ...
  local plys = GRPCore.Game.GetPlayers()
...
function MFD:Awake(...)
    while not GRPCore do Citizen.Wait(0); end
    while not GRPCore.IsPlayerLoaded() do Citizen.Wait(0); end
    GRPCore.TriggerServerCallback('MF_DopePlant:GetStartData', function(retVal) self.dS = true; self.cS = retVal; end)
...
AddEventHandler("Weed20gToWeed4g", function()
	...
	if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		exports["grp-taskbar"]:taskBar(15000, "Weed Oz  >  Weed Hq")
		Citizen.Wait(15000)
...
AddEventHandler("Weed4gToJoint2g", function()
	...
	if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
		TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
		exports["grp-taskbar"]:taskBar(15000, "Weed Hq  >  2g Joint")
		Citizen.Wait(15000)
		ClearPedTasks(PlayerPedId())
	else
		exports["grp-taskbar"]:taskBar(15000, "Weed Hq  >  2g Joint")

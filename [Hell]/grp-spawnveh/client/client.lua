...
AddEventHandler('VehSpawn', function(vehicle)
	...
	GRPCore.Game.SpawnVehicle(model, coords + 3, heading, function(vehicle)
	end)

end)

RegisterCommand('svems', function(source, args, user)
	if (GRPCore.GetPlayerData().job.name == 'ambulance') then
		...
		if tostring(args[1]) == nil then
			...
			if tostring(args[1]) ~= nil then
				...
				if argh == '1' then
					...
					GRPCore.Game.SpawnVehicle(model, coords, heading, function(vehicle)
					local plate = GetVehicleNumberPlateText(vehicle)
					exports["grp-oGasStations"]:SetFuel(vehicle, 100)
...
RegisterCommand('impound', function(source)
	if GRPCore.GetPlayerData().job.name == 'police' or GRPCore.GetPlayerData().job.name == 'mechanic' or GRPCore.GetPlayerData().job.name == 'ambulance' then
...
RegisterCommand("extra", function(source, args, rawCommand)
  ...
  if PlayerData.job.name == 'police' then
	...
	local veh = GetVehiclePedIsIn(ped, false)
    	...
    	if toggle == "true" then
			toggle = 0
			exports["grp-taskbar"]:taskBar(800, "Adding Extra")
    		end
		if veh ~= nil and veh ~= 0 and veh ~= 1 then
			exports["grp-taskbar"]:taskBar(800, "Removing Extra")
	...
	for k,v in pairs(Config.PoliceGarage) do 
		if IsPedInAnyVehicle(ped, true) then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= Config.Distance then
				...
				exports["grp-taskbar"]:taskBar(5000, "Completing Task")
				TriggerEvent('DoLongHudText', 'Your vehicle has been repaired', 1)
				SetVehicleFixed(veh)
				SetVehicleDirtLevel(veh, 0.0)
				exports["grp-oGasStations"]:SetFuel(veh, 100)
...
AddEventHandler('impoundVeh', function()
	...
	local vehicle, attempt = GRPCore.Game.GetVehicleInDirection(), 0
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		exports["grp-taskbar"]:taskBar(3000, "Impounding")
...
RegisterCommand('tint', function(source, args, raw)
	if GRPCore.GetPlayerData().job.name == 'police' or GRPCore.GetPlayerData().job.name == 'ambulance' then
		...
		exports["grp-taskbar"]:taskBar(800, "Tinting Vehicle")
...
RegisterCommand('livery', function(source, args, raw)
    ...
    if tonumber(args[1]) ~= nil and GRPCore.GetPlayerData().job.name == 'police' or GRPCore.GetPlayerData().job.name == 'ambulance' and GetVehicleLiveryCount(vehicle) - 1 >= tonumber(args[1]) then

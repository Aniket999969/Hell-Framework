	    	...
	    	for i = 1, #drugLocs do
				...
				if dstcheck < 5.0 then
				    ...
				    if IsControlJustReleased(0,38) then
		            	...
		            	elseif exports['grp-inventory']:hasEnoughOfItem('oxy').count >= 12 then
...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["grp-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
		        ...
		        if exports["grp-inventory"]:hasEnoughOfItem("rollcash",10,false) then     
		        ...
		        elseif exports["grp-inventory"]:hasEnoughOfItem("markedbills",1,false) then     

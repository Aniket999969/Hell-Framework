...
listOn = false
	while true do
		...
		if IsControlJustPressed(1, 38) and IsPedInAnyVehicle(GetPlayerPed(-1), false) ~= 1 then
			...
			if GetDistanceBetweenCoords(PlayerPos, -1172.6, -1572.13, 4.67, true) <= 5 then						
				-- selling bud
				local smallbud = exports["grp-inventory"]:hasEnoughOfItem("smallbud",5,false)
				if smallbud then
			...
			if GetDistanceBetweenCoords(PlayerPos, 3803.58, 4444.46, 4.1, true) <= 15 then						
				...
				local wetbud = exports["grp-inventory"]:hasEnoughOfItem("wetbud",5,false)
					if wetbud then

					local finished = exports["grp-taskbar"]:taskBar(10000,"Drying Weed",false,false,playerVeh)

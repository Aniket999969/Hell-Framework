	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["grp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["grp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
function unholster1h(weaponHash)
	...
	if weaponHash ~= -538741184 and weaponHash ~= 615608432 then
        ...
        GRPCore.TriggerServerCallback('grp-inventory:getAmmoCount', function(ammoCount)
...
function updateAmmo()
    ...
    if type(ammoType) == 'number' then 
        ...
        TriggerServerEvent('grp-inventory:updateAmmoCount', hash, ammoCount)

...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		GRPCore.TriggerServerCallback('grp-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('grp-ambulancejob:setDeathState', true)
...
function releaseBody()
    ...
    GRPCore.TriggerServerCallback("grp-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		GRPCore.TriggerServerCallback('grp-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = GRPCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = GRPCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', GRPCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('grp-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('grp-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		GRPCore.TriggerServerCallback('grp-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			GRPCore.SetPlayerData('lastPosition', playerpos)
			GRPCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('grp:updateLastPosition', playerpos)
...
RegisterNetEvent('grp-ambulancejob:useItem')
AddEventHandler('grp-ambulancejob:useItem', function(itemName)
	GRPCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["grp-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('grp-ambulancejob:heal', 'big', true)
			TriggerEvent('grp-hospital:client:RemoveBleed')
			TriggerEvent('grp-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["grp-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('grp-ambulancejob:heal', 'small', true)
			TriggerEvent('grp-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('grp-hospital:client:RemoveBleed', ped) 
	TriggerEvent('grp-hospital:client:ResetLimbs', ped)

	GRPCore.UI.Menu.CloseAll()
end

AddEventHandler('grp:onPlayerDeath', function(data)
	OnPlayerDeath()
end)
...
RegisterNetEvent('grp-ambulancejob:revive')
AddEventHandler('grp-ambulancejob:revive', function()
	attemptRevive()
end)

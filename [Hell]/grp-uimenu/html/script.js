...
$(document).ready(function() {
    ...
    $("body").on("keyup", function (key) {
        if (closeKey.includes(key.which)) {
            $(".navigation").fadeOut();
            $.post(`http://grp-uimenu/closeUI`, JSON.stringify({value: false}));
        }
    });
    ...
    $("#backbtn").on("click", function(event) {
        $(".navigation").fadeOut();
        $(".mainmenu").empty();
        $.post(`http://grp-uimenu/selectedValue`, JSON.stringify({value: "back"}));
    })
    ...
    $(".mainmenu").on("click", ".item", function(event) {
        ...
        if (menuType == "list" || menuType == "hover") {
            ...
            $.post(`http://grp-uimenu/selectedValue`, JSON.stringify({value: (typeof thisval == "object") ? JSON.parse(thisval) : thisval}));
        }
        ...
        if (menuType == "multi") {
            ...
            let thisval = (typeof $(this).data("value") == "number") ? $(this).data("value"): window.atob($(this).data("value"))
            $.post(`http://grp-uimenu/hoveredValue`, JSON.stringify({value: (typeof thisval == "object") ? JSON.parse(thisval) : thisval, event: $(this).data("event")}));
        }

...
RegisterNetEvent("grp-fleeca:openDoor_c")
AddEventHandler("grp-fleeca:openDoor_c", function(coords, method)
    if method == 1 then
...
RegisterNetEvent("grp-fleeca:resetDoorState")
AddEventHandler("grp-fleeca:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("grp-fleeca:lootup_c")
AddEventHandler("grp-fleeca:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("grp-fleeca:outcome")
AddEventHandler("grp-fleeca:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("grp-fleeca:startheist", fleeca.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("grp-fleeca:startLoot_c")
AddEventHandler("grp-fleeca:startLoot_c", function(data, name)
    --local check = true
    --[[while check do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 0.75 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("grp-fleeca:lootup", name, "Loot1")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("grp-fleeca:lootup", name, "Loot2")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("grp-fleeca:lootup", name, "Loot3")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot1 and LootCheck[name].Loot2 and LootCheck[name].Loot3) then
                        ...
                        if initiator then
                            TriggerEvent("grp-fleeca:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("grp-fleeca:stopHeist_c")
AddEventHandler("grp-fleeca:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

RegisterNetEvent("grp-fleeca:policenotify")
AddEventHandler("grp-fleeca:policenotify", function(name)
    local PlayerData = GRPCore.GetPlayerData()
    local blip = nil
...
AddEventHandler("grp-fleeca:freezeDoors", function()
    Citizen.CreateThread(function()
        while true do
            local pedcoords = GetEntityCoords(PlayerPedId())
            ...
            for k, v in pairs(fleeca.Banks) do
                if Freeze[k] < 3 then
                    ...
                    if dst < 10 then
                        local obj = GRPCore.Game.GetClosestObject(fleeca.door, vector3(v.doors.secondloc.x, v.doors.secondloc.y, v.doors.secondloc.z))
...
AddEventHandler("grp-fleeca:reset", function(name, data)
    ...
    if data.hash == nil then
        TriggerServerEvent("grp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 4)
    elseif not data.hash == nil then
        TriggerServerEvent("grp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 5)
    end
    TriggerEvent("grp-fleeca:cleanUp", data, name)
end)

AddEventHandler("grp-fleeca:startheist", function(data, name)
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    if data.hash == nil then
    TriggerServerEvent("grp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 1)
    elseif data.hash ~= nil then
        TriggerServerEvent("grp-fleeca:openDoor", "anana", 2)
    end
    startdstcheck = true
    disableinput = false
    currentname = name
...
AddEventHandler("grp-fleeca:cleanUp", function(data, name)
    Citizen.Wait(60000)
    ...
    TriggerServerEvent("grp-fleeca:setCooldown", name)
    initiator = false
end)
...
function SecondDoor(data)
    ...
    exports["grp-taskbar"]:taskBar(1500, "Using Malicious Card")
        ...
        disableinput = false
        exports["grp-taskbar"]:taskBar(2000, "Accessing the Panel")
        TriggerEvent('DoLongHudText', 'Access complete!', 1)
        PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
        TriggerServerEvent("grp-fleeca:openDoor", vector3(data.doors.secondloc.x, data.doors.secondloc.y, data.doors.secondloc.z), 3)
        disableinput = false
end
...
function SpawnTrolleys(data, name)
    ...
    local players, nearbyPlayer = GRPCore.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 20.0)
    ...
    TriggerServerEvent("grp-fleeca:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("grp-fleeca:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
    ...
    if not done then
        ...
        if dst < 4 then
            ...
            if dst < 0.75 and IsControlJustReleased(0 , 38) then
                if exports['grp-inventory']:hasEnoughOfItem('cb', 1) then
                        done = true
                        TriggerEvent('inventory:removeItem', 'cb', 1)
                        return SecondDoor(fleeca.Banks[currentname])
                    elseif not exports['grp-inventory']:hasEnoughOfItem('cb', 0) then
...
Citizen.CreateThread(function()
    ...
    GRPCore.TriggerServerCallback("grp-fleeca:getBanks", function(result)
        fleeca.Banks = result
    end)
    TriggerEvent("grp-fleeca:freezeDoors")
    Citizen.Wait(1000)
    while true do
            ...
            if not v.onaction then
                ...
                if dst <= 5 and not Check[k] then
                    ...
                    if dst <= 1 and IsControlJustReleased(0, 38) then
                        ...
                        local outcome = exports['grp-thermite']:startGame(1,1,7,500)
                        if outcome then
                            FreezeEntityPosition(PlayerPedId(), false)
                            TriggerServerEvent("grp-fleeca:startcheck", k)
                        else
                            if exports['grp-inventory']:hasEnoughOfItem('thermite', 1) then
                                TriggerEvent('inventory:removeItem', 'thermite', 1)
                                local coords = GetEntityCoords(PlayerPedId())
                                exports['grp-thermite']:startFireAtLocation(coords.x, coords.y, coords.z - 1, 10000)
                            else
                                FreezeEntityPosition(PlayerPedId(), false)
                            end
                        end

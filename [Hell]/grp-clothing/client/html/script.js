...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('http://grp-clothing/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://grp-clothing/togglecursor', JSON.stringify({}));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://grp-clothing/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('http://grp-clothing/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('http://grp-clothing/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('http://grp-clothing/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://grp-clothing/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://grp-clothing/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://grp-clothing/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://grp-clothing/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://grp-clothing/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://grp-clothing/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://grp-clothing/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('http://grp-clothing/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('http://grp-clothing/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('http://grp-clothing/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://grp-clothing/savehaircolor', JSON.stringify({
                ...
                $.post('http://grp-clothing/saveeyecolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://grp-clothing/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),

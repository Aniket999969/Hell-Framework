...
AddEventHandler('grp-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("grp-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("grp-clothing:insert_character_face", data)
            TriggerServerEvent("grp-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("grp-clothing:guardarSkin")
AddEventHandler("grp-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("grp-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("grp-clothing:insert_character_face", data)
...
RegisterNetEvent("grp-clothing:inService")
AddEventHandler("grp-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('grp-identity:identityCheck')
AddEventHandler('grp-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("grp-clothing:hasEnough")
AddEventHandler("grp-clothing:hasEnough", function(menu)
...
RegisterNetEvent("grp-clothing:setclothes")
AddEventHandler("grp-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            GRPCore.TriggerServerCallback('grp-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
            ...
            DoScreenFadeIn(50)
            Wait(5000)
            if not exports["grp-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["grp-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
RegisterNetEvent("grp-clothing:defaultReset")
AddEventHandler("grp-clothing:defaultReset", function()
    local gender = nil
    GRPCore.TriggerServerCallback('grp-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("grp-clothing:settattoos")
AddEventHandler("grp-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("grp-clothing:setpedfeatures")
AddEventHandler("grp-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if exports["grp-housing"]:imClosesToRoom2() or exports["grp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('grp-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if exports["grp-housing"]:imClosesToRoom2() or exports["grp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('grp-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if exports["grp-housing"]:imClosesToRoom2() or exports["grp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('grp-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports["grp-housing"]:imClosesToRoom2() or exports["grp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('grp-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('grp-clothing:listOutfits')
AddEventHandler('grp-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('grp-clothing:outfits')
AddEventHandler('grp-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("grp-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("grp-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("grp-clothing:get_outfit", pId)
    else
        TriggerServerEvent("grp-clothing:list_outfits")
    end
end)

...
function MenuVehicleSpawner()
	...
	GRPCore.UI.Menu.Open(
		...
		function(data, menu)
			GRPCore.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				if data.current.value == 'phantom3' then
					GRPCore.Game.SpawnVehicle("trailers2", Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(trailer)
...
function SelectBinandCrew()
	...
	local NewBin, NewBinDistance = GRPCore.Game.GetClosestObject(Config.DumpstersAvaialbe)
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			GRPCore.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Throw bag in truck.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					GRPCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Clean up area", 1.0)		
					else
					GRPCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Collection done.. Wait on truck..", 1.0)		
					end
				else
					GRPCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Collect bag from Trashbin ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end

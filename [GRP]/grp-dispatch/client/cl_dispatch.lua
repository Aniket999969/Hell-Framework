...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
  local currentJob = GRPCore.GetPlayerData().job.name
            ...
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications then
                    ...
                    elseif currentJob == "news" then
                        if exports["grp-inventory"]:getQuantity("scanner") > 0 then
                            local newsObject = {}
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 178) and (GRPCore.GetPlayerData().job.name == 'ambulance' or GRPCore.GetPlayerData().job.name == 'police') and not exports["grp-ambulancejob"]:GetDeath()) then
...
AddEventHandler("civilian:alertPolice",function(basedistance,alertType,objPassed,isGunshot,isSpeeder)
    ...
    if (alertType == "gunshot" or alertType == "gunshotvehicle") and not GRPCore.GetPlayerData().job.name == 'police' then
      AlertGunShot()
    end
    ...
    if alertType == "robberyhouse" and not GRPCore.GetPlayerData().job.name == 'police' then
      AlertCheckRobbery2()
    end
end)
...
AddEventHandler('police:panic', function()
      ...
      recipientList = {
		...
		TriggerEvent('grp-alerts:policepanic')
end)
...
AddEventHandler("police:1047", function()
    ...
    recipientList = {
      police = "police", ambulance = "ambulance"
		...
		TriggerEvent('grp-alerts:downguy')
end)
...
function AlertGunShot()
    Citizen.CreateThread(function() 
      ...
      TriggerEvent('grp-alerts:gunshot')
...
function CarCrash()
  ...
  TriggerEvent('grp-alerts:vehiclecrash')
...
function AlertCheckLockpick(object)
  ...
  if not DoesEntityExist(targetVehicle) then
    ...
    TriggerEvent('grp-alerts:stolenveh')
  end
end
...
function AlertpersonRobbed(vehicle)
  ...
  TriggerEvent('grp-alerts:robstore')
  Wait(math.random(5000,15000))
...
function AlertCheckRobbery2()
  ...
  TriggerEvent('grp-alerts:robhouse')
  Wait(math.random(5000,15000))
...
function AlertJewelRob()
  ...
  TriggerEvent('grp-alerts:jewrob')
  Wait(math.random(5000,15000))
...
RegisterNetEvent('grp-dispatch:drugjob')
AddEventHandler("grp-dispatch:drugjob",function()
  AlertDrugJob()
end)

RegisterNetEvent('grp-dispatch:bankrobbery')
AddEventHandler("grp-dispatch:bankrobbery",function()
  AlertFleecaRobbery()
end)

RegisterNetEvent('grp-dispatch:jailbreak')
AddEventHandler("grp-dispatch:jailbreak",function()
  AlertJailBreak()
end)

RegisterNetEvent('grp-dispatch:jewelrobbery')
AddEventHandler("grp-dispatch:jewelrobbery",function()
  AlertJewelRob()
  return
end)

RegisterNetEvent('grp-dispatch:houserobbery')
AddEventHandler("grp-dispatch:houserobbery",function()
  AlertCheckRobbery2()
end)

RegisterNetEvent('grp-dispatch:storerobbery')
AddEventHandler("grp-dispatch:storerobbery",function()
  AlertpersonRobbed(vehicle)
end)

RegisterNetEvent('grp-dispatch:carjacking')
AddEventHandler("grp-dispatch:carjacking",function()
  AlertCheckLockpick(object)
end)

RegisterNetEvent('grp-ambulancejob:downplayer')
AddEventHandler("grp-ambulancejob:downplayer",function()
    AlertDeath()
end)

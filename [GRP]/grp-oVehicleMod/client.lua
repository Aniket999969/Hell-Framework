...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        disableTurning()
        local finished = exports["grp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["grp-taskbar"]:taskBar(5000,"Taking off Harness",true)
    else
        finished = exports["grp-taskbar"]:taskBar(5000,"Putting on Harness",true)
    end
    if (finished == 100) then

...
TryToCrack = function(safeEntity)
	...
	GRPCore.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		if state then
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks < Config.TotalLocks then
						...
						GRPCore.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								local breachObjects = SpawnBreachObjects(safeEntity)
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		GRPCore.TriggerServerCallback("safebreach:receiveReward", function(received)
			if received then

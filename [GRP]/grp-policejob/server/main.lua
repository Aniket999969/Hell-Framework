...
if Config.MaxInService ~= -1 then
	TriggerEvent('grp-service:activateService', 'police', Config.MaxInService)
end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		local user = GRPCore.GetPlayerFromId(_source)
		local player = user.identifier
		-- Is it worth telling all clients to refresh?
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('grp-policejob:updateBlip', -1)
...
RegisterServerEvent('grp-policejob:spawned')
AddEventHandler('grp-policejob:spawned', function()
	local _source = source
	local xPlayer = GRPCore.GetPlayerFromId(_source)
...
RegisterServerEvent('grp-policejob:forceBlip')
AddEventHandler('grp-policejob:forceBlip', function()
	TriggerClientEvent('grp-policejob:updateBlip', -1)
end)
...
AddEventHandler('policejob:payBill', function(args)
	local src = source
	local xPlayer = GRPCore.GetPlayerFromId(args[1])
...
AddEventHandler('reviveGranted', function(target)
	local xPlayer = GRPCore.GetPlayerFromId(source)
		...
		--TriggerClientEvent('GRPCore_ambulancejob:revive', target)
		Wait(5000)
...
RegisterCommand('givelicense', function(source, cb)
	...
	for k, v in ipairs(players) do
	  ...
	  local xSource = GRPCore.GetPlayerFromId(source)
		...
		TriggerEvent('grp-license:checkLicense', v, 'weapon', function(cb)
		  if cb == false then 
			TriggerEvent('grp-license:addLicense', v, 'weapon', function()
			end)

...
AddEventHandler('grp:setJob', function(job)
	...
	TriggerServerEvent('grp-policejob:forceBlip')
end)
...
AddEventHandler('grp-phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'police' and PlayerData.job.name == dispatchNumber then
		-- if GRPCore_service is enabled
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["grp-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
			TriggerServerEvent("grp-policejob:checkgsr", GetPlayerServerId(t))
		end
	end
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not exports["grp-ambulancejob"]:GetDeath() then
		finished = exports["grp-taskbarskill"]:taskBar(1200,7)
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		if Config.MaxInService ~= -1 then
			TriggerServerEvent('grp-service:disableService', 'police')
		end
	end
end)
...
Citizen.CreateThread(function() 
	...
	while true do
	  ...
	  if handCuffedWalking or handCuffed then
		  ...
		  DisableControlAction(1, 37, true) --Disables INPUT_SELECT_WEAPON (tab) Actions
		  DisablePlayerFiring(PlayerPedId(), true) -- Disable weapon firing
		  local dead = exports["grp-ambulancejob"]:GetDeath()
...
AddEventHandler( 'Police:Radio', function()
	if isCop then
	    ...
	    if ( DoesEntityExist( ped ) and not IsEntityDead( ped )) and exports["grp-ambulancejob"]:GetDeath() ~= 1 then
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ambulance" then
				local bandages = exports["grp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("police:rob", function()
	if not exports["grp-ambulancejob"]:GetDeath() then
		...
		local finished = exports["grp-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["grp-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["grp-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh(ent)
	...
	local finished = exports["grp-taskbar"]:taskBar(1000,"Impounding",false,true)	
...
RegisterNetEvent('grp-policejob:updateBlip')
AddEventHandler('grp-policejob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then
		GRPCore.TriggerServerCallback('grp-society:getOnlinePlayers', function(players)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('grp-policejob:spawned')
	end
...
AddEventHandler("tp:pdrevive", function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
		...
		TriggerServerEvent('grp-ambulancejob:revivePD', GetPlayerServerId(closestPlayer))

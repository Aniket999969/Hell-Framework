...
AddEventHandler( 'cell:doors', function(num)
    ...
    TriggerServerEvent("grp-doors:alterlockstate",tonumber(num))
end)
...
Citizen.CreateThread(function()  -- Calc door around player into an array , dist of 100 every 5 seconds
    ...
    plyCoords = GetEntityCoords(playerped)
    while true do
        for i = 1, #GRP.DoorCoords do
            local distance = #(plyCoords - vector3(GRP.DoorCoords[i]["x"], GRP.DoorCoords[i]["y"], GRP.DoorCoords[i]["z"]))
            if distance < distanceCheckStage1 then
                closeDoorsStage1[i] = GRP.DoorCoords[i]
            else
    ...
    while true do
        ...
        if currentTargetDist < distvariable and targetDoor ~= nil then    
            ...
            if cZ(objCoords["z"],targetDoor["z"],limit) then
                ...
                if(GRP.offSet[targetDoor["doorType"]]) then
                    if (type(targetDoor["doorType"]) == "number") then
                        local pos = GRP.offSet[targetDoor["doorType"]]
                ...
                if  IsControlJustReleased(1,  Controlkey["generalUse"][1]) then
                    if OpenCheck(curClosestNum) then
                        ...
                        if shortcheck > 2.2 and isKeyDoor(curClosestNum) then
                            ...
                            TriggerServerEvent("grp-doors:alterlockstate",curClosestNum)
                        end
                    end
                end 
            end     
...
function OpenCheck(curClosestNum)
    ...
    for k,v in pairs(GRP.rankCheck) do
...
function isKeyDoor(num)
    ...
    if GRP.DoorCoords[num]["doorType"] == "prop_gate_prison_01" then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == 1286392437 then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == "v_ilev_fin_vaultdoor" then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == "prop_facgate_08" then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == "hei_prop_station_gate" then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == -1603817716 then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == 1501451068 then
        return false
    end
    if GRP.DoorCoords[num]["doorType"] == -550347177 then
        return false
    end
    return true
end

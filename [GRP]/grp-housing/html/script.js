...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('http://grp-housing/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null) {
                $.post('http://grp-housing/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('http://grp-housing/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("http://grp-housing/spawnobject", JSON.stringify({
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('http://grp-housing/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post('http://grp-housing/buySelectedObject');
    selectedObjectData = null;
...
$(document).on('click', '#cancel-object', function(){
    $.post('http://grp-housing/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('http://grp-housing/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("http://grp-housing/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        }
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('http://grp-housing/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('http://grp-housing/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    $(".decorate-confirm").css("display", "none");
    selectedObjectData = null;
    $.post("http://grp-housing/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('http://grp-housing/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('http://grp-housing/exit', JSON.stringify({}));
});
GRP.Admin.Menu = {}
GRP._Admin.Menu = {}
GRP._Admin.Menu.PlayerOptions = {}
GRP._Admin.Menu.SearchOptions = {
    type = nil,
    data = nil
}
GRP._Admin.Menu.Target = {}
...
function GRP.Admin.Menu.Init(self)
...
function GRP.Admin.Menu.SetSubMenuProperties(self, menu)
    WarMenu.SetMenuWidth(menu, 0.5)
    WarMenu.SetTitleColor(menu, 135, 206, 250, 255)
    WarMenu.SetTitleBackgroundColor(menu, 0 , 0, 0, 150)
    WarMenu.SetMenuBackgroundColor(menu, 0, 0, 0, 100)
...
function GRP.Admin.Menu.DrawCommand(self, cmd)
    if not cmd or not GRP.Admin:CommandExists(cmd) then return end

    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            if WarMenu.IsMenuOpened("command") then GRP.Admin:GetCommandData(cmd).drawcommand() else return end
            if WarMenu.MenuButton("Back", "acommands") then return end
        end
    end)
end

function GRP.Admin.Menu.DrawTargets(self, cmd, cb)
    WarMenu.OpenMenu("targetmenu")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(GRP._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
                if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) GRP.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) GRP.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
end

function GRP.Admin.Menu.DrawTextInput(self, defaultText, cb)
    Citizen.CreateThread(function()
...
function GRP.Admin.Menu.ShowTextEntry(self, title, subMsg, cb)
    SendNUIMessage({open = true, textEntry = true, title = title, submsg = subMsg and subMsg or ""})
    textEntryCb = function(text) cb(text) end
end

function GRP.Admin.Menu.DrawRanks(self, cmd, cb)
    WarMenu.OpenMenu("ranklist")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("ranklist") do
            Citizen.Wait(0)
            for k,v in spairs(GRP.Admin:GetRanks(), function(t, a, b) return t[a].grant < t[b].grant end) do
                if WarMenu.MenuButton(k, "command") then self:DrawCommand(cmd) cb(k) GRP.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) GRP.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()        
        for k,v in spairs(GRP._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.Button("[" .. v.src .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then GRP._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(GRP._Admin.DiscPlayers, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "aplayeropts") then GRP._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
        if WarMenu.MenuButton("Back", "aplayers") then return end
    end

    local function DrawPlayerOptions()
        local options = GRP._Admin.Menu.PlayerOptions
        ...
        if options then
            ...
            if GRP.Admin:IsSuperAdmin() then
    ...
    local function DrawCommands()
        local userRank = GRP.Admin:GetPlayerRank()

        if cat then
            for k,v in spairs(GRP.Admin:GetCommands(), function(t, a, b) return t[a].title < t[b].title end) do
                if v.category == cat then
                    if GRP.Admin:RankHasCommand(userRank, v.command) then if WarMenu.Button(v.title) then WarMenu.OpenMenu("command") GRP.Admin.Menu:DrawCommand(v.command) end end
                end
            end
        end
    ...
    local function DrawCategories()
        for k,v in spairs(GRP.Admin:GetCategories(), function(t, a, b) return a < b end) do
    ...
    local function DrawDev()
        local userRank = GRP.Admin:GetPlayerRank()
        if WarMenu.Button("Dev mode: " .. (GRP._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "superadmin" then 
            GRP._Admin.Menu.DevMode = not GRP._Admin.Menu.DevMode 
            TriggerEvent("grp-admin:currentDevmode",GRP._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (GRP._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "superadmin" then 
            GRP._Admin.Menu.DevDebug = not GRP._Admin.Menu.DevDebug 
            TriggerEvent("grp-admin:currentDebug",GRP._Admin.Menu.DevDebug)
            TriggerEvent("hud:enabledebug", nil,GRP._Admin.Menu.DevDebug)
        end
        if WarMenu.MenuButton("Back", "amenu") then return end
    end
...
RegisterNetEvent("grp-admin:openMenu")
AddEventHandler("grp-admin:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterCommand('menu', function()
    if group ~= "user" then
        TriggerEvent('grp-admin:openMenu')
    else

...
RegisterNetEvent("grp-state:stateSet")
AddEventHandler("grp-state:stateSet",function(stateId,stateLength)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedInMeleeCombat(PlayerPedId()) then
        	...
        	if target == lastTarget or lastTarget == nil then
				if IsPedAPlayer(target) then
					TriggerEvent("grp-state:stateSet",1,300)
        			lastTarget = target
        		end
        	else
				if IsPedAPlayer(target) then
        ...
        if IsPedSwimming(PlayerPedId()) then
        	...
        	if speed > 0 then
        		sweatTriggered = true
        		TriggerEvent("grp-state:stateSet",20,0)
        		TriggerEvent("grp-state:stateSet",21,0)
        		TriggerEvent("grp-state:stateSet",23,600)
        		if bodySweat < 180000 then
        			bodySweat = bodySweat + (150 + math.ceil(speed * 40))
        		else
        			bodySweat = bodySweat + (150 + math.ceil(speed * 11))
        		...
        		if bodySweat > 210000 then
        			TriggerEvent("grp-state:stateSet",19,600)
	        		bodySweat = 210000
	        	end
        	end
        end

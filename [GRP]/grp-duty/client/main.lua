...
----markers
AddEventHandler('grp-duty:hasEnteredMarker', function (zone)
...
AddEventHandler('grp-duty:hasExitedMarker', function (zone)
  CurrentAction = nil
end)
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
        ...
        GRPCore.Game.Utils.DrawText3D(vector3(v.Pos.x, v.Pos.y, v.Pos.z), closestString, 0.4)
      end
    end
  end
end)
...
Citizen.CreateThread(function ()
  while true do
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('grp-duty:hasEnteredMarker', currentZone)
    end

    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('grp-duty:hasExitedMarker', LastZone)
    end
  end
end)
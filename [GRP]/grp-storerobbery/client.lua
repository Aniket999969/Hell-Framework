...
RegisterNetEvent('grp-storerobbery:onPedDeath')
AddEventHandler('grp-storerobbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('grp-storerobbery:msgPolice')
AddEventHandler('grp-storerobbery:msgPolice', function(store, robber)
    --TriggerEvent('grp-alerts:storerob')
    TriggerEvent('grp-dispatch:storerobbery')
end)

RegisterNetEvent('grp-storerobbery:removePickup')
AddEventHandler('grp-storerobbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('grp-storerobbery:robberyOver')
AddEventHandler('grp-storerobbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('grp-storerobbery:talk')
AddEventHandler('grp-storerobbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('grp-storerobbery:rob')
AddEventHandler('grp-storerobbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('grp-storerobbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('grp-storerobbery:resetStore')
AddEventHandler('grp-storerobbery:resetStore', function(i)
    while not GRPCoreLoaded do Wait(0) end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('grp-storerobbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            GRPCore.TriggerServerCallback('grp-storerobbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('grp-storerobbery:notif', i)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('grp-storerobbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('grp-storerobbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end

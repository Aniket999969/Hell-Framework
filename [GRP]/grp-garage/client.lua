...
RegisterNetEvent('grp-garages:client:houseGarageConfig')
AddEventHandler('grp-garages:client:houseGarageConfig', function(garageConfig)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('grp-garage:putcaringarage')
	end
...
function RentrerVehicule()
	...
	--TriggerServerEvent('grp-garage:CheckForVeh')
...
function ListeVehicule(page)
	...
	ClearMenu()
	local carCount = 0
	GRPCore.TriggerServerCallback('grp-garage:getOwnedCars', function(ownedCars)
		   ...
		   local vehicle_state = value.state
			...
			--if garages[selectedGarage].garage == curGarage then
					...
					if vehicle_state == "Standard Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $500", " Engine %:" .. GRPCore.Math.Round(enginePercent,1) .. "", " Body %:" .. GRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif vehicle_state == "Police Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $1500", " Engine %:" .. GRPCore.Math.Round(enginePercent,1) .. ""," Body %:" .. GRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" and curGarage ~= "house" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, curGarage, " Engine %:" .. GRPCore.Math.Round(enginePercent,1) .. "", " Body %:" .. GRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					--elseif currentGarage == ""
					else

						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, tostring(value.state) , " Engine %:" .. GRPCore.Math.Round(enginePercent,1) .. ""," Body %:" .. GRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					end
				end
			--end
		   count = count + 1
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz)
	...
	GRPCore.Game.SpawnVehicle(vehicle.model, {
		x = spawnx,
		y = spawny,
		z = spawx
		...
		--GRPCore.Game.SetVehicleProperties(callback_vehicle, vehicle)
	...
	TriggerServerEvent('grp-advancedgarage:setVehicleState', plate, false)
end
...
SetVehicleProperties = function(vehicle, vehicleProps, fuelSet)
    GRPCore.Game.SetVehicleProperties(vehicle, vehicleProps)

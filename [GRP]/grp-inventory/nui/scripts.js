...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://grp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
      } else {
        $.post("http://grp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
      }
    }
  })
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
  ...
  $.post("http://grp-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
  ...
  $.post("http://grp-inventory/move", JSON.stringify(arr));
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
  ...
  $.post("http://grp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
  let arr = itemList[itemid].craft
  let amount = moveAmount;
  $.post("http://grp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function BuildDrop(brokenSlots) {
  $.post("http://grp-inventory/dropIncorrectItems", JSON.stringify({
    slots: brokenSlots
  }));
}
  ...
  for (i = 0; i < slotLimit + 1; i++) {
    ...
    if (isEmpty(inventory[i])) {
      ...
      if (itemList[itemid] !== undefined) {
        ...
        $.post("http://grp-inventory/updateMyQuality", JSON.stringify({
          item_id: itemid,
          slot: slot,
          quality: quality
        }));
...
function UpdateSetWeights() {
  ...
  $.post("http://grp-inventory/Weight", JSON.stringify({
    weight: meh
  }));
}
...
function DragToggle(slot,using) {
  ...
  if (slot) {
    ...
    if (occupiedslot == true && dragging == false) {
      ...
      $.post("http://grp-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
  ...
  $.post("http://grp-inventory/SlotJustUsed", JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
  // $.post('http://grp-inventory/swapstack', JSON.stringify({
  //   slot1: slot1,
  //   slot2: slot2,
  //   inv1: inv1,
  //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
  ...
  $.post("http://grp-inventory/ServerCloseInventory", JSON.stringify({
    name: TargetInventoryName
  }));
  TargetInventoryName = "none";
  $.post("http://grp-inventory/Close", JSON.stringify({
...
function useitem() {
  ...
  if (dragging != false) {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
      ...
      $.post("http://grp-inventory/invuse", JSON.stringify(arr));

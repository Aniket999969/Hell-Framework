...
GRPCore.RegisterServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
GRPCore.RegisterServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
GRPCore.RegisterServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
GRPCore.RegisterServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
GRPCore.RegisterServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = GRPCore.GetPlayerFromId(src)
    if Player ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM apartments WHERE  identifier = @identifier",{['@identifier'] = Player.identifier}, function(result)
...
GRPCore.RegisterServerCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = GRPCore.GetPlayerFromId(src)

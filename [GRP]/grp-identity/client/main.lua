...
RegisterNetEvent('grp-identity:showRegisterIdentity')
AddEventHandler('grp-identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)
...
RegisterNetEvent('grp-identity:saveID')
AddEventHandler('grp-identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
    ...
    if reason == "" then
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent('grp-identity:setIdentity', data, myIdentifiers)
        EnableGui(false)
		TriggerEvent('grp-clothing:defaultReset', myIdentifiers.id)

...
function bankBalance(player)
  return GRPCore.GetPlayerFromId(player).getBank()
end
...
function deposit(player, amount)
  ...
  local user = GRPCore.GetPlayerFromId(player)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = GRPCore.GetPlayerFromId(to)
	local balance = 0
...
TriggerEvent('es:addCommand', 'givecash', function(source, args)
  ...
  local amount = args[2]
  local user = GRPCore.GetPlayerFromId(sender)


  local playerRec = GRPCore.GetPlayerFromId(reciever)
...
AddEventHandler("cash:remove", function(user, amount)
  local _source = source
  local xPlayer = GRPCore.GetPlayerFromId(user)

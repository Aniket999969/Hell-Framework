...
RegisterNUICallback("ban", function(data,cb)
	...
	GRPCore.TriggerServerCallback("el_bwh:ban",function(success,reason)
		if success then GRPCore.ShowNotification("~g~Successfully banned player") else GRPCore.ShowNotification(reason) end -- dont ask why i did it this way, im a bit retarded
...
RegisterNUICallback("warn", function(data,cb)
	...
	GRPCore.TriggerServerCallback("el_bwh:warn",function(success)
		if success then GRPCore.ShowNotification("~g~Successfully warned player") else GRPCore.ShowNotification("~r~Something went wrong") end
	end, data.target, data.message, data.anon)
end)

RegisterNUICallback("unban", function(data,cb)
	if not data.id then return end
	GRPCore.TriggerServerCallback("el_bwh:unban",function(success)
		if success then GRPCore.ShowNotification("~g~Successfully unbanned player") else GRPCore.ShowNotification("~r~Something went wrong") end
	end, data.id)
end)

RegisterNUICallback("getListData", function(data,cb)
	if not data.list or not data.page then cb(nil); return end
	GRPCore.TriggerServerCallback("el_bwh:getListData",function(data)
		cb(data)
...
AddEventHandler("el_bwh:acceptedAssist",function(t)
	...
	if target then
		...
		assisting = true
		assist_target = t
		GRPCore.Game.Teleport(ped,GetEntityCoords(GetPlayerPed(target))+vector3(0,0.5,0))
	end
end)
...
AddEventHandler("el_bwh:assistDone",function()
	if assisting then
		assisting = false
		if pos_before_assist~=nil then GRPCore.Game.Teleport(GetPlayerPed(-1),pos_before_assist+vector3(0,0.5,0)); pos_before_assist = nil end
		assist_target = nil
	end
end)
...
AddEventHandler("el_bwh:showWindow",function(win)
	...
	elseif win=="banlist" or win=="warnlist" then
		...
		GRPCore.TriggerServerCallback(win=="banlist" and "el_bwh:getBanList" or "el_bwh:getWarnList",function(list,pages)

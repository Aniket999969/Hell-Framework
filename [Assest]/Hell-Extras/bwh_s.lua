...
Citizen.CreateThread(function() -- startup
    ...
    while GRPCore==nil do Wait(0) end
    ...
    GRPCore.RegisterServerCallback("el_bwh:ban", function(source,cb,target,reason,length,offline)
        if not target or not reason then return end
        local xPlayer = GRPCore.GetPlayerFromId(source)
        local xTarget = GRPCore.GetPlayerFromId(target)
    ...
    GRPCore.RegisterServerCallback("el_bwh:warn",function(source,cb,target,message,anon)
    ...
    GRPCore.RegisterServerCallback("el_bwh:getWarnList",function(source,cb)
        local xPlayer = GRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
            local warnlist = {}
    ...
    GRPCore.RegisterServerCallback("el_bwh:getBanList",function(source,cb)
        local xPlayer = GRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
    ...
    GRPCore.RegisterServerCallback("el_bwh:getListData",function(source,cb,list,page)
        local xPlayer = GRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
    ...
    GRPCore.RegisterServerCallback("el_bwh:unban",function(source,cb,id)
        local xPlayer = GRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
...
function logAdmin(msg)
    for k,v in ipairs(GRPCore.GetPlayers()) do
        if isAdmin(GRPCore.GetPlayerFromId(v)) then
...
function banPlayer(xPlayer,xTarget,reason,length,offline)
    ...
    MySQL.Async.execute("INSERT INTO bwh_bans(id,receiver,sender,length,reason) VALUES(NULL,@receiver,@sender,@length,@reason)",{["@receiver"]=json.encode(targetidentifiers),["@sender"]=xPlayer.identifier,["@length"]=length,["@reason"]=reason},function(_)
        ...
        if offline then xTarget = GRPCore.GetPlayerFromIdentifier(xTarget) end -- just in case the player is on the server, you never know
        if xTarget then
...
AddEventHandler("el_bwh:acceptAssistKey",function(target)
    ...
    acceptAssist(GRPCore.GetPlayerFromId(_source),target)
end)
